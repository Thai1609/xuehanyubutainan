...
import { Topic, DialogueLine, Vocabulary } from '../data/chineseData';
...
export default function TopicDetail({ 
  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-4">
      <button 
        onClick={onBack}
        className="mb-10 px-6 py-2 rounded-full bg-white border border-orange-100 text-sm font-bold text-orange-600 hover:bg-orange-50 flex items-center gap-2 w-fit transition-colors shadow-sm"
      >
      ...
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-5xl font-black text-gray-900 mb-4 tracking-tighter">{topic.title}</h1>
          <p className="text-xl text-gray-500 max-w-2xl">{topic.description}</p>
        </div>
        <div className="flex gap-3">
           <span className="px-4 py-2 bg-orange-100 text-orange-700 rounded-2xl text-sm font-bold uppercase tracking-widest">{topic.level}</span>
        </div>
      </div>

      {topic.lessons && (
        <div className="mb-10 flex flex-col items-center bg-white p-6 rounded-[40px] border border-orange-50 shadow-sm relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-orange-400 to-orange-100" />
          
          <div className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-300 mb-3">Chọn bài học</div>
          ...
          <div className="flex gap-3 w-full max-w-3xl">
            <button 
              ...
              className="p-4 rounded-3xl bg-orange-100 text-orange-600 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-orange-200 transition-all"
              title="Bài trước"
            >
            ...
            <div className="relative flex-1">
              <select
                ...
                className="w-full appearance-none bg-orange-50/50 border-2 border-orange-100 text-gray-800 py-4 pl-6 pr-12 rounded-3xl font-black text-lg focus:outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100 transition-all cursor-pointer hover:bg-orange-50"
              >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        ...
        <div className="flex flex-wrap items-center gap-2 bg-white p-2 rounded-3xl border border-orange-50 shadow-sm">
          <button 
      ...
      <AnimatePresence mode="wait">
        ...
        >
            ...
            {activeTab === 'radicals' && (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
                {topic.radicals.map((r, i) => (
                  <div key={i} className="bg-white p-8 rounded-[40px] border-b-4 border-green-100 shadow-sm flex flex-col items-center text-center group hover:border-green-300 transition-all">
            ...
            {activeTab === 'vocab' && (
              <div className="space-y-8">
                {/* Vocab Controls */}
                <div className="flex flex-col md:flex-row gap-4 items-center justify-end bg-white p-6 rounded-[32px] border border-orange-50 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    {filteredVocab.map((v, i) => (
                      <motion.div 
                        ...
                        key={i} 
                        className="bg-white p-8 rounded-[40px] border-b-4 border-orange-100 shadow-sm group hover:border-orange-300 transition-all relative overflow-hidden"
                      >
                          ...
                          <div className="flex items-center gap-6">
                            <div className="flex-shrink-0">
                              <StrokeWriter 
                                ...
                                size={80} 
                                className="bg-orange-50 rounded-2xl p-2 border border-orange-100 group-hover:border-orange-200 transition-all" 
                              />
                            </div>
                            <div>
                              <div className={cn("text-gray-400 font-mono text-sm mb-1 transition-all", hidePinyin && "opacity-0 blur-sm select-none")}>{v.pinyin}</div>
                              <div className={cn("text-2xl font-bold text-gray-800 transition-all", hideTranslation && "opacity-0 blur-sm select-none")}>{v.meaning}</div>
                            </div>
                          </div>
                          <button 
                        ...
                        <div className="bg-gray-50 p-5 rounded-3xl border-2 border-dashed border-orange-100 relative group/ex">
                          <div className={cn("text-base font-medium text-gray-700 mb-2 leading-relaxed pr-8 transition-all", hideTranslation && "opacity-0 blur-sm select-none")}>{v.example}</div>
                          {v.examplePinyin && (
                            <div className={cn("text-sm text-orange-400 font-mono mb-2 transition-all", hidePinyin && "opacity-0 blur-sm select-none")}>{v.examplePinyin}</div>
                          )}
                          <div className={cn("text-sm text-gray-400 italic transition-all", hideTranslation && "opacity-0 blur-sm select-none")}>{v.exampleTranslation}</div>
                          <button 
                            onClick={() => playAudio(v.example)}
                            className="absolute top-4 right-4 text-gray-300 hover:text-orange-500 transition-colors"
                          >
                            <Volume2 size={16} />
                          </button>
                        </div>
        ...
        {activeTab === 'grammar' && (
          <div className="space-y-8 max-w-5xl mx-auto">
            {activeGrammar.map((g, i) => (
              <div key={i} className="bg-purple-50 p-10 rounded-[48px] border-2 border-purple-100">
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-12 h-12 bg-purple-200 text-purple-700 rounded-2xl flex items-center justify-center text-xl shadow-inner">
                    💡
                  </div>
                  <h3 className="text-2xl font-bold text-purple-900 tracking-tight">{g.rule}</h3>
                </div>
                <p className="text-purple-800 mb-10 text-lg leading-relaxed">{g.description}</p>
                <div className="space-y-6">
                   {g.examples.map((ex, j) => (
                    <div key={j} className="bg-white p-6 rounded-3xl shadow-sm border border-purple-100 flex gap-6 items-start group relative">
                       <div className="w-8 h-8 rounded-full bg-purple-50 text-purple-500 flex items-center justify-center text-xs font-bold shrink-0 mt-1">
                         {j + 1}
                       </div>
                       <div className="flex-1">
                         <div className="flex items-center justify-between mb-1">
                           <div className="text-xl font-serif text-gray-900 tracking-wide">{ex.chinese}</div>
                           <button 
                         ...
                         <div className={cn("text-purple-400 font-mono text-sm mb-2 uppercase tracking-tighter transition-all", hidePinyin && "opacity-0 blur-sm select-none")}>{ex.pinyin}</div>
                         <div className={cn("text-gray-500 font-medium transition-all", hideTranslation && "opacity-0 blur-sm select-none")}>{ex.translation}</div>
                       </div>
                    </div>
                  ))}
                </div>
