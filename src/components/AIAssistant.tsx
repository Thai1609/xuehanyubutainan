...
import { chatWithTutorStream } from '../services/ai';
...
export default function AIAssistant() {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await chatWithTutorStream(userMessage, history, level, (chunk) => {
        setStreamingMessage(chunk);
      });
      setMessages(prev => [...prev, { role: 'model', text: response || 'Lỗi kết nối' }]);
    } catch (e) {
      setMessages(prev => [...prev, { role: 'model', text: 'Lỗi thiết lập AI, kiểm tra khóa API của bạn nhé.' }]);
    }
  ...
  return (
    <div className="flex flex-col h-[700px] border border-orange-100 rounded-[40px] bg-white shadow-2xl overflow-hidden relative">
      {/* Header */}
      ...
      >
        ...
        {(isLoading || streamingMessage) && (
          ...
          >
            <div className="w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 mt-1 shadow-sm font-bold text-lg bg-green-100 text-green-600 self-start">
              汉
            </div>
            <div className="max-w-[75%] rounded-[28px] px-6 py-4 text-sm shadow-sm leading-relaxed relative group bg-white border border-gray-100 text-gray-700 rounded-tl-none min-w-[120px]">
              {streamingMessage ? (
                <div className="prose prose-sm prose-slate max-w-none dark:prose-invert prose-headings:text-inherit">
